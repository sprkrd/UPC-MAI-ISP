...
from .attackers.BlackBoxAttacker import GANAttack, WhiteNoiseAttack, PGDAttackBB
...
if __name__ == "__main__":
    ...
    # which=3 alternative basic model (used for non-targetted attacks)
    ...
    model_att = pretrained_res18(which=3, gpu=runGPU)
    attacker_mild_bb = PGDAttackBB(model_att, k=5, epsilon=0.03)
    attacker_strong_bb = PGDAttackBB(model_att, k=5, epsilon=0.03)

    evaluate(cnn, test_set, prefix="Test set (mild non-directed PGD)", attacker=attacker_mild_bb, attack_mode="black")
    evaluate(cnn, test_set, prefix="Test set (strong non-directed PGD)", attacker=attacker_strong_bb, attack_mode="black")
